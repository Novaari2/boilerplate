...
func (c *Client) Call(_ context.Context, method, url string, body io.Reader, headers map[string]string, response interface{}) error {
	...
	if err != nil {
		return &Error{
			Message:  fmt.Sprintf("unable to send request: %s", err.Error()),
			RawError: err,
		}
	}
